...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return buildPageMetadata({
    locale,
    routeKey: 'initiativeProof',
    title: dict.proof.metadata_title,
    description: dict.proof.metadata_description,
  });
}

export default async function ProofPage({ params }: { params: Promise<{ lang: Locale }> }) {
  const { lang } = await params;
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 md:px-12">
      ...
      <div className="space-y-20 pb-24">
        ...
        <ScrollReveal>
          <section className="border-t border-alesvia-muted/10 pt-16">
            <div className="glass rounded-2xl p-10 md:p-16 border border-alesvia-muted/10">
              <SectionLabel>{d.cta_label}</SectionLabel>
              <p className="text-alesvia-muted text-lg leading-relaxed max-w-2xl mb-8">{d.cta_text}</p>
              <Link href={getPublicPath(lang, 'contact')} className="btn-primary bg-alesvia-primary text-alesvia-bg font-semibold px-8 py-4 rounded-lg inline-block">
                {d.cta_label}
              </Link>
            </div>
          </section>
