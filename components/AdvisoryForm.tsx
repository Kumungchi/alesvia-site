...
export default function AdvisoryForm({ dict, lang }: { dict: AdvisoryFormDict; lang: string }) {
  ...
  return (
    <div className="bg-alesvia-surface/95 backdrop-blur-md rounded-2xl p-8 md:p-10 border border-alesvia-muted/20 shadow-2xl relative overflow-hidden">
      {/* Subtle institutional accent line at the top instead of a massive glowing orb */}
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-alesvia-accent to-alesvia-primary" />
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className={inputGroupClass}>
              <label htmlFor="adv-name" className={labelClass}>{dict.name}</label>
              <input id="adv-name" name="name" type="text" required className={inputClass} />
            </div>
            <div className={inputGroupClass}>
              <label htmlFor="adv-email" className={labelClass}>{dict.email}</label>
              <input id="adv-email" name="email" type="email" required className={inputClass} />
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className={inputGroupClass}>
              <label htmlFor="adv-org-name" className={labelClass}>{dict.org_name}</label>
              <input id="adv-org-name" name="org_name" type="text" required className={inputClass} />
            </div>
            <div className={inputGroupClass}>
              <label htmlFor="adv-org-type" className={labelClass}>{dict.org_type}</label>
              <select id="adv-org-type" name="org_type" required className={`${inputClass} appearance-none cursor-pointer`} defaultValue="">
                <option value="" disabled hidden></option>
                <option value={dict.type_startup}>{dict.type_startup}</option>
                <option value={dict.type_vc}>{dict.type_vc}</option>
                <option value={dict.type_other}>{dict.type_other}</option>
              </select>
            </div>
          </div>

          <div className={inputGroupClass}>
            <label htmlFor="adv-stage" className={labelClass}>{dict.stage}</label>
            <select id="adv-stage" name="stage" required className={`${inputClass} appearance-none cursor-pointer`} defaultValue="">
              <option value="" disabled hidden></option>
              <option value={dict.stage_preseed}>{dict.stage_preseed}</option>
              <option value={dict.stage_seed}>{dict.stage_seed}</option>
              <option value={dict.stage_series}>{dict.stage_series}</option>
              <option value={dict.stage_na}>{dict.stage_na}</option>
            </select>
          </div>

          <div className={inputGroupClass}>
            <label htmlFor="adv-message" className={labelClass}>{dict.message}</label>
            <textarea id="adv-message" name="message" required rows={4} className={`${inputClass} resize-y min-h-[100px]`} />
          </div>
